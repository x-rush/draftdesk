...
export const workspaceViews=["discover","library","trends","ideas","people","activities","chat","plans","sources","runs","skills","connections","settings"];
export function readWorkspaceLocation(search:string){
 ...
 return {activityPlatform:["哔哩哔哩","抖音","快手","小红书"].includes(p.get("activityPlatform")||"")?p.get("activityPlatform")!:"all",activityTime:["ongoing","upcoming","ended","unknown"].includes(p.get("activityTime")||"")?p.get("activityTime")!:"all",view:workspaceViews.includes(p.get("view")||"")?p.get("view")!:"discover",q:(p.get("q")||"").slice(0,500),kind:["all","news","topic","trend","idea","person","activity"].includes(p.get("kind")||"")?p.get("kind")!:"all",quality:["active","all","ready","review","rejected"].includes(p.get("quality")||"")?p.get("quality")!:"active",page:page("page"),jobsPage:page("jobsPage"),receiptsPage:page("receiptsPage"),creation:Object.keys(creationLabels).includes(p.get("creation")||"")?p.get("creation")!:"all",jobId:p.get("jobId")||"",runId:p.get("runId")||""};
}
