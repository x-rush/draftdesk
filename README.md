...
4. 创作活动：在自己的 Chrome / Edge 登录 B站、抖音、快手、小红书官方活动中心，使用工作台提供的开源扩展按关键词读取列表和可见详情。扩展按平台保留结果，分别导出 JSON 后在工作台合并预览；过期、未知截止时间和只有摘要的活动不会进入付费分析。只有活动中心链接而无单条规则链接的结果需按标题回原平台核对。参见 [四平台活动采集说明](docs/creator-activities.md)。
