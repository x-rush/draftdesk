...
2. activityUrl 必须逐字来自输入证据 URL，优先完整活动规则页。若官方活动日历只有弹窗而没有单条链接，可用活动中心 URL，但必须在证据中保留活动标题和完整弹窗规则，并标记登录核实与按标题定位；不能把中心链接描述成直达链接。域名属于平台不等于作者官方：普通用户爆料是 lead；创作者中心账号定向活动需登录核实，不声称全部覆盖，不推测用户有资格。
