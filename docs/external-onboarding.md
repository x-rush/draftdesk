...
本机 2026-09-24 试跑：两个容器均能访问 SearXNG 与工作台，令牌预检通过；Hermes 一次搜索返回 10 条线索，筛出 1 条官方来源，经宿主机适配器提交得到 `pending-review` 回执；OpenClaw 首个过宽任务超时，改为新会话、限定一次搜索后取得同一官方来源，日期精度不符被预检拒绝，适配器移除仅日期的 `publishedAt` 后提交成功。两包均仅含证据，无草稿。当时 Hermes 仅配置了 SearXNG 搜索，未配置正文提取。摘要材料不得冒充全文或直接支撑高质量选题。外部模型费用不受 DraftDesk 内置预算约束。
