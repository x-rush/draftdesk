...
同机 Docker 的另一种交付方式是让 Agent 返回标准 JSON，再由宿主机的 `agents/sync-result.ps1 -Agent openclaw|hermes -ResponseFile <保存的最终响应> -CredentialFile <忽略目录内的令牌文件>` 做协议预检和提交。这样提交令牌无需放进 Agent 容器；适配器也支持从容器内的绝对路径 `-ResultPath` 读取文件。这是可选的本机安全适配器，不是用户必须采用的接入方式。OpenClaw 的运行中 Gateway 不支持同时使用 `agent --local`；每次研究指定新的 session ID，避免旧会话历史带来高额 token 消耗。Hermes 单次命令可能拦截脚本写文件，使用最终 JSON 响应即可，无需关闭安全限制。
