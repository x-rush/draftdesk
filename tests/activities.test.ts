...
test("老库增加活动策略且不覆盖用户调整，平台过滤在分页之前",()=>{const dir=mkdtempSync(path.join(tmpdir(),"dd-activity-"));let db=new Store(dir);try{const plan=defaultPlans.find(p=>p.kind==="activities")!;assert.equal(queryPlan(plan,6).slice(0,4).length,4);assert.equal(plan.scheduleEnabled,false);db.put("plans",plan.id,{...plan,name:"我的活动雷达"});db.saveArtifact(activity,"activity-test","review",[]);db.close();db=new Store(dir);assert.equal((db.get<any>("plans",plan.id)).name,"我的活动雷达");assert.equal((db.snapshot(new URLSearchParams("page=1&view=activities&activityPlatform=小红书")) as any).artifacts.length,0);assert.equal((db.snapshot(new URLSearchParams("page=1&view=activities&activityPlatform=哔哩哔哩")) as any).artifacts.length,1);}finally{db.close();rmSync(dir,{recursive:true,force:true});}});
...
test("活动证据经整理、专项分析、审核后进入活动列表（模拟模型）",async()=>{
 ...
 assert.equal(db.get<any>("jobs",j.id).state,"completed");assert.equal(calls,3);const a=(db.snapshot(new URLSearchParams("page=1&view=activities")) as any).artifacts[0];assert.equal(a.kind,"activity");assert.equal(a.details.directions.length,3);assert.equal(a.visibility,"private");assert.equal(a.quality,"review");
